...
`qofeno <command> [options]` — every command also has `-h`.
...
Headless runs are **fail-closed**: tools execute only when permission rules exist (`qofeno permissions grant …`) or policy allows; no prompts are ever shown.

## Management commands

```
sessions list | rm <id> | export <id> [path]
config get <key> | path | policy
